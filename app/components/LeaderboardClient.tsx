...
export default function LeaderboardClient({
  ...
  const matchupOnlySettlements = minimizeSettlements(players, matchupPayouts.net)
                            ...
                            {groupHasDaytona && <>
                              <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                                ...
                                {scFrontNine.map((h) => {
                                  ...
                                  return <td key={h.hole_number} style={tdSc()}>{pts === null ? <span style={{ color: '#d1d5db' }}>–</span> : <span style={{ display: 'inline-flex', fontVariantNumeric: 'tabular-nums', fontWeight: 600, color: pColor(pts), fontSize: '0.7rem' }}>{pts !== 0 && <span style={{ width: '1ch', textAlign: 'right', flexShrink: 0 }}>{pts > 0 ? '+' : '-'}</span>}<span>{pts === 0 ? '0' : String(Math.abs(pts))}</span></span>}</td>
                                })}
                                <td style={tdSc(true)}>{frontPts === null ? <span style={{ color: '#d1d5db' }}>–</span> : <span style={{ display: 'inline-flex', fontVariantNumeric: 'tabular-nums', fontWeight: 700, color: pColor(frontPts) }}>{frontPts !== 0 && <span style={{ width: '1ch', textAlign: 'right', flexShrink: 0 }}>{frontPts > 0 ? '+' : '-'}</span>}<span>{frontPts === 0 ? '0' : String(Math.abs(frontPts))}</span></span>}</td>
                                {scBackNine.map((h) => {
                                  const pts = holePtsMaps.get(h.hole_number)?.has(row.player.id) ? holePtsMaps.get(h.hole_number)!.get(row.player.id)! : null
                                  return <td key={h.hole_number} style={tdSc()}>{pts === null ? <span style={{ color: '#d1d5db' }}>–</span> : <span style={{ display: 'inline-flex', fontVariantNumeric: 'tabular-nums', fontWeight: 600, color: pColor(pts), fontSize: '0.7rem' }}>{pts !== 0 && <span style={{ width: '1ch', textAlign: 'right', flexShrink: 0 }}>{pts > 0 ? '+' : '-'}</span>}<span>{pts === 0 ? '0' : String(Math.abs(pts))}</span></span>}</td>
                                })}
                                <td style={tdSc(true)}>{backPts === null ? <span style={{ color: '#d1d5db' }}>–</span> : <span style={{ display: 'inline-flex', fontVariantNumeric: 'tabular-nums', fontWeight: 700, color: pColor(backPts) }}>{backPts !== 0 && <span style={{ width: '1ch', textAlign: 'right', flexShrink: 0 }}>{backPts > 0 ? '+' : '-'}</span>}<span>{backPts === 0 ? '0' : String(Math.abs(backPts))}</span></span>}</td>
                                <td style={tdSc()}>{totalPts === null ? <span style={{ color: '#d1d5db' }}>–</span> : <span style={{ display: 'inline-flex', fontVariantNumeric: 'tabular-nums', fontWeight: 700, color: pColor(totalPts) }}>{totalPts !== 0 && <span style={{ width: '1ch', textAlign: 'right', flexShrink: 0 }}>{totalPts > 0 ? '+' : '-'}</span>}<span>{totalPts === 0 ? '0' : String(Math.abs(totalPts))}</span></span>}</td>
                              </tr>
                              {groupHasDaytona && (
                                <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
