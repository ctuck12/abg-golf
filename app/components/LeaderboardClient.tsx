      ...
      <div className="max-w-lg mx-auto px-4 pt-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          {rows.map((row, i) => {
            ...
            return (
              <a key={row.team.id} href={`/scorecard/${row.team.id}`}
                className="flex items-center px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition"
                style={isLeader ? { background: '#fef9e7' } : {}}>
                <span className="w-6 mr-3 text-sm font-bold" style={{ color: isLeader ? gold : '#9ca3af' }}>
                  {hasScores ? i + 1 : '–'}
                </span>
                <span className="flex-1 font-semibold text-gray-900 text-sm truncate">{row.team.name}</span>
                {Array.from({ length: ballsCount }, (_, bi) => (
                  <span key={bi} className="text-center text-sm" style={{ width: colWidth }}>
                    <ScoreCell vp={row.summary.ballVsPar[bi]} />
                  </span>
                ))}
                <span className="text-center text-sm text-gray-500" style={{ width: '3rem' }}>
        ...
        <p className="text-center text-xs text-gray-400 mt-3">Tap a team to view their scorecard</p>
