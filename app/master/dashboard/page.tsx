...
export default async function MasterDashboardPage() {
  ...
  const [{ data: orgs }, { data: courses }, { data: activeRounds }] = await Promise.all([
    ...
    sb.from('courses').select('id, name, slug, pars, stroke_indexes, is_active, course_tees(id, name, course_rating, slope_rating, ghin_course_id, ghin_tee_set_id, position)').order('name'),
    sb.from('rounds')
