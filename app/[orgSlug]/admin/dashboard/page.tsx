...
export default async function OrgAdminDashboardPage({ params }: { params: Promise<{ orgSlug: string }> }) {
  ...
  const [teamsRes, holesRes, ballValuesRes] = await Promise.all([
    roundId ? sb.from('teams').select('id, name, pin, is_admin, daytona_variant, banker_side_game, banker_side_game_min_bet, auto_strokes, hammer_side_game, hammer_base_bet, hammer_format').eq('round_id', roundId).order('name') : Promise.resolve({ data: [] }),
