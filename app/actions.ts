...
export async function createRound(_prev: unknown, formData: FormData) {
  ...
  try {
    ...
    // Which holes offer the manual 2x doublers; unset reads as par 3s only
    const bankerDoubleScope = isBanker ? ((formData.get('banker_double_scope') as string) === 'all' ? 'all' : 'par3') : null
...
export async function addTeam(_prev: unknown, formData: FormData) {
  ...
  const bankerDoubleScope = (formData.get('banker_double_scope') as string) === 'all' ? 'all' : 'par3'
...
export async function updatePlayingGroupSettings(groupId: string, settings: {
  ...
  banker_double_scope?: string | null
